...
%t3-t1 <= 0;
[x, fval, reason, output, last_gener] = ga(@test_func_Q3, 3, [-1, 1,1],[0],Aeq,beq,lb,rb,@nonlcon, options_1);

% last_gener:当前最后一次搜索的种群样本，在此基础再次搜索
options_2 = gaoptimset('Generations', T/2, 'InitialPopulation', last_gener, 'PlotFcns', @gaplotbestf);
[x, fval, reason, output, last_gener] = ga(@test_func_Q3, 3, [-1, 1,1],[0],Aeq,beq,lb,rb,@nonlcon, options_2);%接力进化
% Bestx = x
% BestFval = fval
